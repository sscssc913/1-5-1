...
def main():
    ...
    train_thickness, train_lab, scaler_thickness, encoder_materials1, encoder_materials2, scaler_lab, \
    ...
    val_thickness, val_lab, _, _, _, _, val_materials1_encoded, val_materials2_encoded = \
        load_and_process_data(r'D:\pytorch\INN\valid_dataset_100k.csv', scaler_thickness, encoder_materials1, encoder_materials2, scaler_lab, fit_scaler=False)
    ...
    val_thickness_tensor = torch.tensor(val_thickness, dtype=torch.float32)
    val_materials1_tensor = torch.tensor(val_materials1_encoded, dtype=torch.long)
    val_materials2_tensor = torch.tensor(val_materials2_encoded, dtype=torch.long)
    val_lab_tensor = torch.tensor(val_lab, dtype=torch.float32)


    train_dataset = TensorDataset(train_thickness_tensor, train_materials1_tensor, train_materials2_tensor, train_lab_tensor)
    train_loader = DataLoader(train_dataset, batch_size=1024, shuffle=True, num_workers=8, pin_memory=True)
    val_dataset = TensorDataset(val_thickness_tensor, val_materials1_tensor, val_materials2_tensor, val_lab_tensor)
    ...
    fnn.load_state_dict(torch.load(r'D:\pytorch\INN\fnn_7_500_model-integer encoded.pth', map_location=device))
    fnn.eval()
    ...
    # 训练循环
    epochs = 1000
    materials_epochs = 100
    thickness_epochs = 100
    ...
    for epoch in tqdm(range(1, epochs + 1), desc="Training Progress", total=epochs):
        ...
        tqdm.write(
            f'Epoch {epoch}/{epochs}: Training Loss: {avg_train_loss:.8f}, ' +
            ('Accuracy: {:.8f}, '.format(avg_accuracy) if calculate_accuracy else '') +
            f'Thickness Loss: {avg_losses_thickness:.8f}, ' +
            f'Materials Loss: {avg_losses_materials:.8f}, LAB Loss: {avg_losses_lab:.8f}')
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    pred_thickness, pred_material_logits = inn(batch_lab)
                    pred_lab = fnn(pred_thickness,
                                   torch.argmax(pred_material_logits[:, :12], dim=1),
                                   torch.argmax(pred_material_logits[:, 12:], dim=1))

                    # 计算验证损失
                    val_loss, _, _, _ = inn_loss_function(
                        pred_thickness, batch_thickness, pred_material_logits,
                        batch_materials1_encoded, batch_materials2_encoded, pred_lab, batch_lab,
                        criterion_mse, criterion_ce, num_materials, weight_thickness, weight_materials, weight_lab
                    )
                    epoch_val_loss += val_loss.item()

            # 打印验证损失
            avg_val_loss = epoch_val_loss / len(val_loader)
            tqdm.write(f'Epoch {epoch}: Validation Loss: {avg_val_loss:.8f}')
    ...
    loss_data.to_csv('integer encoded uniform 7-250.csv', index=False)
